...
const url2 = "https://api.themoviedb.org/3/movie/popular?api_key=" + API_KEY +
...
const url4 = "https://api.themoviedb.org/3/trending/movie/day?api_key=" + API_KEY;
let trending_movies = axios.get(url4);

const url5 = "https://api.themoviedb.org/3/tv/popular?api_key=" + API_KEY + "&language=en-US&page=1";
let popular_tv = axios.get(url5);

const url6 = "https://api.themoviedb.org/3/tv/top_rated?api_key=" + API_KEY + "&language=en-US&page=1";
...
Promise.all([curr_movies, popular_movies, top_rated_movies, trending_movies, popular_tv, top_rated_tv, trending_tv])
    .then((responses) => {
        responses.forEach((resp) => {
            resp_list = resp.data["results"].map(({ id, title, name, poster_path }) => ({ id, title, name, poster_path }));
        })

        // atneratively, do all this data cleanup on the client side. i.e. just send 'responses' as one big object to the client side and cleanup afterwards.
    })
    .then((resp_list) => {
